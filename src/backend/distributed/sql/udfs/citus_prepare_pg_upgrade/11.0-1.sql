CREATE OR REPLACE FUNCTION pg_catalog.citus_prepare_pg_upgrade()
    ...
    AS $cppu$
DECLARE
    citus_depends_on_citus_stat_activity boolean;
BEGIN
    ...
    SELECT count(*) > 0
    FROM pg_depend
    WHERE objid::regclass::text = 'citus_stat_activity'
    AND refobjid IN (
        SELECT oid FROM pg_extension WHERE extname = 'citus'
    )
    INTO citus_depends_on_citus_stat_activity;

    -- drop citus_stat_activity to recreate in citus_prepare_pg_upgrade
    -- so it has all the columns in pg_stat_activity, even if the upgrade changes them
    -- also we need to check if it exists to make this function idempotent
    IF citus_depends_on_citus_stat_activity THEN
        ALTER EXTENSION citus DROP VIEW citus_stat_activity;
        ALTER EXTENSION citus DROP TYPE citus_stat_activity_one_node;
    END IF;
